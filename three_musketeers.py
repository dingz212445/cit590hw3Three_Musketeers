...
# by David Matuszek and Ding Zhang.
...
def has_some_legal_move_somewhere(who):
    ...
    the legal move is."""
    assert who == 'M' or who == 'R'

    location_list = all_locations()
    for loc in location_list:
        if at(loc) == who and can_move_piece_at(loc):
            return True
...
def is_legal_location(location):
    ...
    if location[0] < 0 or location[1] > 4 or location[1] < 0 or location[1] > 4:
        return False
...
def is_within_board(location, direction):
    ...
    if location[0] == 0 and direction == 'up':
        return False
    if location[0] == 4 and direction == 'down':
        return False
    if location[1] == 0 and direction == 'left':
        return False
    if location[1] == 4 and direction == 'right':
        return False
...
def all_possible_moves_for(player):
    ...
    possible_moves = []
    from_locations = all_locations()
    for loc in from_locations:
        if at(loc) == player:
            directions = possible_moves_from(loc)
            for direct in directions:
                possible_moves.append((loc, direct))

    return possible_moves

def make_move(location, direction):
    """Moves the piece in location in the indicated direction."""
    if is_legal_location(location) and is_legal_move(location, direction) and is_within_board(location, direction):
        to_location = adjacent_location(location, direction)
        board[to_location[0]][to_location[1]] = at(location)
    ...
    for loc in locations:
        if at(loc) == 'M':
            if flag == 0:
                ...
                column = loc[1]
                flag = 1
            elif flag == 1 and loc[0] != row and loc[1] != column:
                return False
    return True
